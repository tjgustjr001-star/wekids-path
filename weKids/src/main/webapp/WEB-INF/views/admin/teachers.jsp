...
<c:forEach items="${teacherList}" var="teacher">
	<c:choose>
		...
		<c:when test="${teacher.accountStatus eq 'SUSPENDED'}">
			<c:set var="suspendedDeletedCount"
				value="${suspendedDeletedCount + 1}" />
		</c:when>
	</c:choose>
</c:forEach>
...
<div class="admin-teachers-page">
	...
	<div class="page-header-row">
		...
		<div class="page-header-actions">
			<button type="button" class="outline-btn" id="downloadTeacherListBtn">목록
				다운로드</button>
			<button type="button" class="primary-btn" id="openAddTeacherModalBtn">교사
				계정 생성</button>
		</div>
	</div>
	...
	<div class="teachers-top-grid">
		...
		<section class="panel-card">
			...
			<div class="teacher-total-count">${totalTeacherCount}<span>명</span>
			</div>

			<div class="teacher-stat-group">
				<div class="teacher-stat-row">
					<span>활성 계정</span> <strong>${activeCount}명</strong>
				</div>
				...
				<div class="teacher-stat-row top-gap">
					<span>휴면 계정</span> <strong>${dormantCount}명</strong>
				</div>
				<div class="progress-bar">
				...
				<div class="teacher-stat-row top-gap">
					<span>정지 계정</span> <strong>${suspendedDeletedCount}명</strong>
				</div>
				<div class="progress-bar">
	...
	<section class="teachers-board">
		...
		<div class="teacher-table-wrap">
			<table class="teacher-table" id="teacherTable">
				...
				<tbody>
					<c:forEach items="${teacherList}" var="teacher">
						<tr class="teacher-row" data-id="${teacher.teacherId}"
							data-name="${teacher.teacherName}" data-email="${teacher.email}"
							data-school="${teacher.schoolName}"
							data-status="${teacher.accountStatus}">

							<td class="teacher-name-cell"><a
								href="${pageContext.request.contextPath}/admin/teachers/${teacher.teacherId}">
									${teacher.teacherName} </a></td>
							<td>${teacher.email}</td>
							<td>${teacher.schoolName}</td>
							<td>${teacher.classCount}개</td>
							<td><fmt:formatDate value="${teacher.joinDate}"
							...
							<td class="manage-cell">
								<div class="table-action-wrap">
									<a
										href="${pageContext.request.contextPath}/admin/teachers/${teacher.teacherId}"></a>
									...
									<div class="table-row-menu">
										<form
											...
											<button type="submit" class="teacher-status-action">활성
												처리</button>
										</form>
										...
										<form
											...
											<input type="hidden" name="teacherId"
												value="${teacher.teacherId}"> <input type="hidden"
												name="accountStatus" value="DORMANT">
										...
										<form
											...
											<input type="hidden" name="teacherId"
												value="${teacher.teacherId}"> <input type="hidden"
												name="accountStatus" value="SUSPENDED">
										...
										<form
											...
											<input type="hidden" name="teacherId"
												value="${teacher.teacherId}"> <input type="hidden"
												name="accountStatus" value="DELETED">
											<button type="submit" class="teacher-status-action danger">계정
												삭제</button>
										</form>
									</div>
					...
					<c:if test="${empty teacherList}">
						<tr>
							<td colspan="7" style="text-align: center; padding: 30px;">등록된
								교사 정보가 없습니다.</td>
						</tr>
					</c:if>
				</tbody>
		...
		<div class="teachers-board-footer">
			<span>총 <span id="teacherTotalCount">${totalTeacherCount}</span>명
			</span>
	...
	<div class="teacher-modal-dialog">
		<div class="teacher-modal-header">
			...
			<button type="button" class="close-modal-btn"
				id="closeAddTeacherModalBtn">×</button>
		</div>

		<form id="addTeacherForm" class="teacher-modal-form"
			action="${pageContext.request.contextPath}/admin/teachers/regist"
			method="post">
			<div class="teacher-form-grid">
				<div>
					<label>이름 <span class="required-mark">*</span></label> <input
						type="text" name="teacherName" id="teacherNameInput"
						placeholder="예: 홍길동">
				</div>

				<div>
					<label>로그인 아이디 <span class="required-mark">*</span></label> <input
						type="text" name="loginId" id="teacherLoginIdInput"
						placeholder="예: t006">
				</div>

				<div>
					<label>이메일 <span class="required-mark">*</span></label> <input
						type="email" name="email" id="teacherEmailInput"
						placeholder="teacher@daewoo.edu.kr">
				</div>

				<div>
					<label>초기 비밀번호 <span class="required-mark">*</span></label> <input
						type="text" name="initialPassword" id="teacherPasswordInput"
						placeholder="초기 비밀번호 입력">
				</div>

				<div class="teacher-modal-actions">
					<button type="button" class="modal-cancel-btn"
						id="cancelAddTeacherBtn">취소</button>
